...
def train_gcfn_model_per_fold(X_train_deep, X_train_clin, y_train, pos_w_train, random_seed, epochs=200, patience=35, lr_GCC=0.001): # patence 25
    ...
    model     = GlaucomaChemicalFusionNetwork(num_deep_features     = X_train_deep.shape[1], 
                                              num_clinical_features = X_train_clin.shape[1], 
                                              num_classes           = 2).to(DEVICE)
    criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.0, pos_w_train], dtype=torch.float32).to(DEVICE))
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr_GCC)
    ...
    best_model_state                             = None # To hold our Phase 1 snapshot
    ...
    history_train_acc  = []
    history_val_acc    = []
    print("epochs, patience, and lr =", epochs, patience, lr_GCC)
    for epoch in range(epochs):
        ...
        for bd, bc, by in train_loader:
            ...
            running_train_correct += (preds == by).sum().item() 
        scheduler.step()
        epoch_train_loss = running_train_loss / len(train_loader.dataset)
        epoch_train_acc  = running_train_correct / len(train_loader.dataset) # <--- NEW
        history_train_loss.append(epoch_train_loss)
        history_train_acc.append(epoch_train_acc)

        model.eval()
        with torch.no_grad(): 
            logits   = model(X_val_d, X_val_c) # prediction for val set
            val_loss = criterion(logits, y_val).item()
            history_val_loss.append(val_loss)      # Save val loss
            preds    = torch.argmax(logits, dim=1) # Calculate Validation Accuracy
            acc      = (preds == y_val).float().mean().item()
            history_val_acc.append(acc)

        if val_loss < best_val_loss:
            best_val_loss, epochs_no_improve, best_epoch = val_loss, 0, epoch
            best_model_state                             = copy.deepcopy(model.state_dict()) # Take snapshot
        elif (epochs_no_improve := epochs_no_improve + 1) >= patience: 
            break
        
    model.load_state_dict(best_model_state) # Restore the best Phase 1 snapshot to the 'model' variable
    ...
    full_loader            = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(to_tensor(X_train_deep, s_deep_fin), 
                                                         ...
                                                         batch_size = 32, 
                                                         shuffle    = True)
    final_model     = GlaucomaChemicalFusionNetwork(num_deep_features     = X_train_deep.shape[1], 
                                                    num_clinical_features = X_train_clin.shape[1], 
                                                    num_classes           = 2).to(DEVICE)
    optimizer_final = torch.optim.AdamW(final_model.parameters(), lr=lr_GCC)
    ...
    # return final_model,                                    (s_deep_fin, s_clin_fin), best_epoch + 1, history_train_loss, history_val_loss, history_train_acc, history_val_acc
    return final_model, model, (scaler_deep, scaler_clin), (s_deep_fin, s_clin_fin), best_epoch + 1, history_train_loss, history_val_loss, history_train_acc, history_val_acc
...
def run_k_fold_cv(args, df, save_dir):
    ...
    with open(log_file_path, "w") as f:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(StratifiedKFold(n_splits=args.k_fold, shuffle=True, random_state=args.seed).split(X_deep, y_final)):
            ...
            if args.classifier == "xgboost":
                ...
                # Unpack Phase 1 and Phase 2 separately
                final_model, phase1_model, phase1_scalers, scalers, best_ep, train_losses, val_losses, train_accs, val_accs = train_gcfn_model_per_fold(
                    X_tr_d, X_tr_c, y_tr, pos_w_train, args.seed + fold_idx,  epochs=args.epochs_GCC, patience=args.patience_GCC, lr_GCC=args.lr_GCC)
                
                # 1. Generate and save the Plot with Dual Axes
                ...
                ax2 = ax1.twinx()
                ax2.plot(train_accs, label='Training Accuracy', color='purple', linewidth=2, linestyle=':') # <--- NEW
                ax2.plot(val_accs, label='Validation Accuracy', color='green', linewidth=2, linestyle='-.')
                ax2.set_ylabel('Accuracy', color='black')
                ax2.tick_params(axis='y', labelcolor='black')
                
                # Combine Legends and Add Best Epoch Line
                lines1, labels1 = ax1.get_legend_handles_labels()
                lines2, labels2 = ax2.get_legend_handles_labels()
                ax1.axvline(x=best_ep - 1, color='red', linestyle='--', label=f'Selected Epoch ({best_ep})')
                lines3, labels3 = ax1.get_legend_handles_labels() # Re-grab to include the red line
                
                ax1.legend(lines3 + lines2, labels3 + labels2, loc='center right')
                plt.title(f'GCFN Training/Val Loss & Val Accuracy (Fold {fold_idx + 1})')
                ...
                with torch.no_grad():
                    # Evaluate Phase 2 (Final Retrained Model)
                    probs_test        = torch.softmax(final_model(torch.tensor(scalers[0].transform(X_te_d), dtype=torch.float32).to(DEVICE), 
                                                                  torch.tensor(scalers[1].transform(X_te_c), dtype=torch.float32).to(DEVICE)), 
                                                      dim=1)[:, 1].cpu().numpy()
                    # Evaluate Phase 1 (Checkpoint Model)
                    probs_test_phase1 = torch.softmax(phase1_model(torch.tensor(phase1_scalers[0].transform(X_te_d), dtype=torch.float32).to(DEVICE), 
                                                                   torch.tensor(phase1_scalers[1].transform(X_te_c), dtype=torch.float32).to(DEVICE)),
                                                      dim=1)[:, 1].cpu().numpy()
                # --- SAVE GCC MODEL AND SCALERS ---
                fold_save_path  = os.path.join(weights_dir, f"gcfn_fold_{fold_idx + 1}.pth")
                model_cpu_state = {k: v.cpu() for k, v in final_model.state_dict().items()} # Use final_model
                torch.save({'model_state_dict': model_cpu_state, 
                            'scalers': scalers}, 
                           fold_save_path)
                # print(f"✅ GCC Fold {fold_idx + 1} weights saved to: {os.path.abspath(fold_save_path)}")
            ...
            # Print Phase 1 snapshot vs Phase 2 final ---
            if args.classifier == "gcfn":
                # Calculate Phase 1 Test Accuracy
                p1, r1, t1     = precision_recall_curve(y_te, probs_test_phase1)
                best_thresh1   = t1[np.argmax(2 * p1 * r1 / (p1 + r1 + 1e-8))] if len(t1) > 0 else 0.5
                preds_phase1   = (probs_test_phase1 >= best_thresh1).astype(int)
                phase1_test_acc = accuracy_score(y_te, preds_phase1)

                train_acc_at_best = train_accs[best_ep - 1]
                val_acc_at_best   = val_accs[best_ep - 1]
                
                print(f"   ↳ [Phase 1] Train: {train_acc_at_best:.4f} | Val: {val_acc_at_best:.4f} | Test: {phase1_test_acc:.4f}   --->   [Phase 2 Retrained] Final Test: {metrics['accuracy']:.4f}")
    ...
    return k_fold_results, log_file_path, model if args.classifier == "xgboost" else final_model, scaler if args.classifier == "xgboost" else scalers, X_deep.shape[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_GCC",      type=float, default=0.001)
    parser.add_argument("--epochs_GCC",  type=int,   default=200)
    parser.add_argument("--patience_GCC", type=int,  default=35)
